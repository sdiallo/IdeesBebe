...
describe Category do
  ...
  describe '#all_product' do
    ...
    let(:product) { FactoryGirl.create :product, name: "testtest", category_id: subject.id }
    let(:product2) { FactoryGirl.create :product, name: "testtest1",  category_id: subject.id }
    let(:product3) { FactoryGirl.create :product, name: "testtest2",  category_id: subject.id }
   
    context 'with a main category' do
      let(:category2) { FactoryGirl.create :category, main_category_id: main.id }
      let(:product3) { FactoryGirl.create :product, name: "testtest2",  category_id: category2.id }
